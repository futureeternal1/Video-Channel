...
EOF;
    ...
    if (isset($_POST['verify'])) {
        ...
        // Bypass Envato API call for purchase code verification
        // $url = "https://api.envato.com/v3/market/author/sale?code=".$product_code;
        // $curl = curl_init($url);
        //
        // $header = array();
        // $header[] = 'Authorization: Bearer '.$envatoPersonalToken; // $envatoPersonalToken is likely from config
        ...
        // $product_code is the code entered by the user (already cleaned).
        // $envatoItemId is expected to be defined in included config files (e.g., includes/config.php or includes/constant.php).

        $result['msg'] = '<div class="alert alert-success">License Verification Bypassed! Please proceed.</div>';
        $result['start'] = '<table class="table table-hover">';
        $result['purchase_code'] = '<tr><td>Purchase Code</td><td>:</td><td>'.htmlspecialchars($product_code).' (Bypassed)</td>';
        $result['item_id'] = '<tr><td>Item ID</td><td>:</td><td>'.(isset($envatoItemId) ? htmlspecialchars($envatoItemId) : 'BYPASSED_ITEM_ID').'</td>';
        $result['item_name'] = '<tr><td>Item Name</td><td>:</td><td>Bypassed Item Name</td>';
        $result['buyer'] = '<tr><td>Buyer</td><td>:</td><td>Bypassed Buyer</td>';
        $result['license'] = '<tr><td>License</td><td>:</td><td>Bypassed License Type</td>';
        $result['purchase_date'] = '<tr><td>Purchase Date</td><td>:</td><td>'.date('Y-m-d').'</td>';
        $result['purchase_count'] = '<tr><td>Purchase Count</td><td>:</td><td>1 (Bypassed)</td>'; // Added for consistency
        $result['end'] = '</table>';

        // These values will be submitted and potentially saved to 'tbl_license'
        $result['edt_purchase_code'] = $product_code; // This was cleaned on line 47
        $result['edt_item_id'] = (isset($envatoItemId) ? $envatoItemId : 'BYPASSED_ITEM_ID');
        $result['edt_item_name'] = 'Bypassed Item Name';
        $result['edt_buyer'] = 'Bypassed Buyer';
        $result['edt_license'] = 'Bypassed License Type';
        $result['edt_purchase_date'] = date('Y-m-d');
        $result['show_button'] = '<button type="submit" name="submit" class="button button-rounded btn-block waves-effect waves-float">SUBMIT & PROCEED</button>';
        // --- END BYPASS LOGIC ---
